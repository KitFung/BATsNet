...
float angles_from_degrees(const float &degree) {
  constexpr float per_radian = M_PI / 180.0;
  return degree * per_radian;
}
...
bool RawData::buildTimings() {
  ...
  // hdl32
    ...
    printf("Timings not supported for model %s\n", config_.model.c_str());
  }
  ...
  if (timing_offsets.size()) {
    ...
    printf("NO TIMING OFFSETS CALCULATED. ARE YOU USING A SUPPORTED VELODYNE "
           "SENSOR?\n");
  }
  return false;
}
...
RawData::setup(const velodyne::VelodynePointCloudConf &conf) {
  // 64E
  config_.model = conf.model();
  buildTimings();

  // get path to angles.config file for this device
  config_.calibrationFile = conf.has_calibration();
  if (!conf.has_calibration()) {
    std::cerr << "No calibration angles specified! Using test values!"
              << std::endl;
    exit(1);
  } else {
    config_.calibrationFile = conf.calibration();
  }
  ...
  if (!calibration_.initialized) {
    std::cerr << "Unable to open calibration file: " << config_.calibrationFile
              << std::endl;
    return boost::none;
  }

  std::cout << "Number of lasers: " << calibration_.num_lasers << "."
            << std::endl;

  // Set up cached values for sin and cos of all the possible headings
  for (uint16_t rot_index = 0; rot_index < ROTATION_MAX_UNITS; ++rot_index) {
    float rotation = angles_from_degrees(ROTATION_RESOLUTION * rot_index);
...
int RawData::setupOffline(std::string calibration_file, double max_range_,
  ...
  std::cout << "data ranges to publish: [" << config_.min_range << ", "
 ...
 */
void RawData::unpack(const velodyne::VelodynePacket &pkt,
                     DataContainerBase &data, const double &scan_start_time) {
  ...
  const raw_packet_t *raw = (const raw_packet_t *)pkt_data;
    ...
    for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
      ...
      if ((block.rotation >= config_.min_angle &&
        ...
        data.addPoint(x_coord, y_coord, z_coord, corrections.laser_ring,
                      raw->blocks[i].rotation, distance, intensity, time);
      }
    }
    data.newLine();
  }
 ...
 *  @param pc shared pointer to point cloud (points are appended)
 */
void RawData::unpack_vlp16(const velodyne::VelodynePacket &pkt,
  ...
  for (int block = 0; block < BLOCKS_PER_PACKET; block++) {
    ...
    if (block < (BLOCKS_PER_PACKET - 1)) {
      ...
      if (raw_azimuth_diff <
          0) // raw->blocks[block+1].rotation - raw->blocks[block].rotation < 0)
      {
        // ROS_WARN_STREAM_THROTTLE(
        //     60, "Packet containing angle overflow, first angle: "
