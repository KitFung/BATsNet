...
rslidarDriver::rslidarDriver(Conf conf) {
 ...
 */
bool rslidarDriver::poll(Scan *scan) { // Allocate a new shared pointer for
                                       // zero-copy sharing with other nodelets.
  scan->Clear();
  ...
  } else // standard behaviour
  {
    ...
    for (int i = 0; i < config_.npackets; ++i) {
      while (true) {
        ...
        int rc = msop_input_->getPacket(scan->mutable_packet(i),
                                        config_.time_offset);
        if (rc == 0)
          break; // got a full packet?
        if (rc < 0)
