...
export DEVICE_IP=192.168.100.2
...
# route add -net 10.32.0.0/16 gw 192.168.100.2 dev br0
...
# iptables -A FORWARD -j NFQUEUE --queue-num 3 -m physdev --physdev-is-bridged -p $ENCODED_PROTO ! -s $DEVICE_SUBNET -d 192.168.100.3
iptables -A FORWARD -j NFQUEUE --queue-num 4 -m physdev --physdev-is-bridged -p $ENCODED_PROTO ! -s $DEVICE_SUBNET -d 192.168.100.4
iptables -A FORWARD -j NFQUEUE --queue-num 5 -m physdev --physdev-is-bridged -p $ENCODED_PROTO ! -s $DEVICE_SUBNET -d 192.168.100.5
iptables -A FORWARD -j NFQUEUE --queue-num 6 -m physdev --physdev-is-bridged -p $ENCODED_PROTO ! -s $DEVICE_SUBNET -d 192.168.100.6
iptables -A FORWARD -j NFQUEUE --queue-num 7 -m physdev --physdev-is-bridged -p $ENCODED_PROTO ! -s $DEVICE_SUBNET -d 192.168.100.7
