...
class RadarReader():
    def __init__(self, configFileName, folderName, num):
    ...
    def run(self):
        ...
        logging.info("Radar starts to read data")
    ...
    def readAndParseData68xx(self, Dataport, configParameters):
        ...
        if magicOK:
            ...
            idX += 8
            version = format(
                np.matmul(self.byteBuffer[idX:idX + 4], word), 'x')
            idX += 4
        ...
        return dataOK, frameNumber, detObj, datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
