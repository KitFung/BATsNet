...
class H26xParser:
    ...
    def __init__(self, f, verbose=False, use_bitstream=None):
        ...
        if use_bitstream:
            ...
            if not ext in valid_input_ext:
                raise RuntimeError("Valid input types: " +
                                   str(valid_input_ext))
            bitstream_file = f
    ...
    def set_callback(self, name, fun):
        ...
        if name not in self.VALID_CALLBACKS:
            raise RuntimeError(
                name + " is not a valid callback. Choose one of " + str(self.VALID_CALLBACKS) + ".")
        if not callable(fun):
    ...
    def _get_nalu_positions(self):
        ...
        """
        self.nal_unit_positions = list(self.stream.findall(
            self.START_CODE_PREFIX, bytealigned=True))
        self.short_nal_unit_positions = list(self.stream.findall(
            self.START_CODE_PREFIX_SHORT, bytealigned=True))
        ...
        if not self.nal_unit_positions:
            ...
            extra_nal_unit_pos = set(
                [max(s - 8, 0) for s in self.short_nal_unit_positions]) - set(self.nal_unit_positions)
            if len(extra_nal_unit_pos) and self.nal_unit_positions:
                if self.verbose:
                    print("Warning: 3-byte extra NAL unit start code found")
                self.nal_unit_positions.extend(
                    [s + 8 for s in extra_nal_unit_pos])
    ...
    def parse(self):
        ...
        for current_nalu_pos, next_nalu_pos in zip(self.nal_unit_positions, islice(self.nal_unit_positions, 1, None)):
            ...
            if self.verbose:
                ...
                print("NALU length:\t" + str(next_nalu_bytepos -
                                             current_nalu_bytepos) + " Bytes (including start code)")
...
def main():
    path = "./data/Slice-video-H264-1-1591677459.425053"

    with open(path, "rb") as i:
        ex = H26xParser(None, verbose=True, use_bitstream=i)
        ex.parse()
